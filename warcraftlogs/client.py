...
from warcraftlogs.models.reports import Report
from warcraftlogs.models.parses import Parse

class WarcraftLogsClient:
    HOST = 'http://classic.warcraftlogs.com/v1/'
    ...
    def reports(self, guild, server, region, **params):
        path = 'reports/guild/{}/{}/{}'.format(guild, server, region)
        return [
            Report.from_dict(f, server=server, region=region, guild=guild)
            for f in self._get(path, **params).json()
        ]
    ...
    def parses(self, name, server, region, **params):
        ...
        return [
            Parse.from_dict(f, server=server, region=region, name=name)
            for f in self._get(path, **params).json()
        ]